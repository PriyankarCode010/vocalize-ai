...
export async function POST(request: Request) {
  ...
  console.log("[api/meeting/approve] auth result", auth)
  if (!auth?.user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })

  const body = await request.json().catch(() => ({}))
  console.log("[api/meeting/approve] incoming body", body)
  ...
  console.log("[api/meeting/approve] loaded meeting", meeting)
  ...
  if (error) {
    console.error("[api/meeting/approve] update failed", error)
    return NextResponse.json({ error: error.message }, { status: 400 })
  }
  console.log("[api/meeting/approve] request approved", data)
